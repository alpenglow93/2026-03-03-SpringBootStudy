 ...
 * 	=> null 값이 있는 경우 required = false 를 적어준다
 * 	=> Spring Frameword와 동일
 * 
 * 	ThymeLeaf
...
public class BoardController {
	...
	public String board_list(@RequestParam(value = "page", required = false) String page, Model model) // 값 받을 때 @RequestParam 빠지면 오류
	{
		if(page==null)
			page="1";
	...
	public String board_insert_ok(@ModelAttribute("vo") BoardEntity vo) // vo 단위로 받을때는 @ModelAttribute 사용
	{
	...
	public String board_update_ok(@ModelAttribute("vo") BoardEntity vo, Model model)
	{
		...
		{
			vo.setNo(vo.getNo()); // no가 들어가면 update / 없으면 insert
